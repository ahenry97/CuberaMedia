...
Do not use `Deploy from a branch` for this project. If GitHub Pages is set to publish from `develop`, `main`, or the repository root, GitHub will look for an `index.html`, `index.md`, or `README.md` in that branch folder. That is why the README can open instead of the website. The static website is generated by the workflow and published from the `out/` artifact.
...
To publish the site after changing the Pages source:

1. Merge `develop` into `main` when the change is approved for production.
2. Go to `Actions > Deploy GitHub Pages`.
3. Run the workflow on `main`, or let it run automatically after the merge.
4. Approve the `production` environment deployment when GitHub prompts for review.
